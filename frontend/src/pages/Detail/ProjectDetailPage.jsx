...
export default function ProjectDetailPage() {
  ...
  const loadDetail = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/timesheet/detail/project/${id}`);
      setData(res.data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  if (!data?.project) {
    return (
      <AppLayout>
        <EmptyState icon="bi-folder-x" title="Project not found" text="The project could not be loaded." action={<button className="btn btn-primary" onClick={() => navigate(-1)}>Go Back</button>} />
      </AppLayout>
    );
  }
    ...
    <AppLayout>
      <div className="detail-page">
        <div className="detail-hero">
          <div className="detail-hero-icon" style={{ background: "var(--primary-soft)", color: "var(--primary)" }}>
            <i className="bi bi-folder2-open"></i>
          </div>
          <div className="detail-hero-info">
            <h2 className="detail-hero-title">{project.project_name}</h2>
            <div className="detail-hero-meta">
              <span><i className="bi bi-list-task"></i> {stats?.module_count || 0} modules</span>
              <span><i className="bi bi-people"></i> {stats?.contributor_count || 0} contributors</span>
              <span><i className="bi bi-clock"></i> {fmtHrs(stats?.total_hours)} total hours</span>
            </div>
          </div>
        ...
        <div className="grid-cols-4">
          <div className="kpi-card"><div className="kpi-card-label">Modules</div><div className="kpi-card-value">{stats?.module_count || 0}</div></div>
        ...
        {tab === "overview" && (
          <div className="card">
            ...
            <div className="info-grid">
              <div className="info-item"><div className="info-label">Project Name</div><div className="info-value">{project.project_name}</div></div>
              <div className="info-item"><div className="info-label">Total Modules</div><div className="info-value">{stats?.module_count || 0}</div></div>
